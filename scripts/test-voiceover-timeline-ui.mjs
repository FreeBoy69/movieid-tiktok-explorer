...
try {
  for (const { width, height, theme } of [{ width: 1440, height: 1000, theme: "light" }, { width: 1024, height: 900, theme: "light" }, { width: 390, height: 844, theme: "light" }, { width: 1440, height: 1000, theme: "dark" }]) {
    ...
    await page.getByRole("button", { name: "Openverse", exact: true }).click();
    await page.getByLabel(/^Use /).first().click();
    ...
    assert.equal(await page.getByLabel("Auto place", { exact: true }).isChecked(), true);
    ...
    assert.equal(await page.getByLabel("Include in render", { exact: true }).isChecked(), true);
