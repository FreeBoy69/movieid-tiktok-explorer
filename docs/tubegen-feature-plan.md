...
- **Original soundtrack.** The audio source is the generated voiceover or an uploaded audio/video file, which is converted to WAV and transcribed during auto-split. Auto-split returns timed segments with a mood and music direction for each. Segments can be split, retimed, muted, or removed by hand. Composing uses Google Lyria 3 Pro through OpenRouter (`/chat/completions` with `modalities: ["text","audio"]` and `stream: true`; the model can be overridden with `OPENROUTER_MUSIC_MODEL`). Lyria ignores requested length (a request for 20s returned a 61s MP3 on 2026-09-23), so each unmuted segment gets its own cue. The streamed audio is assembled whatever its format (WAV, WAV pieces, MP3, or bare PCM; Lyria returned MP3 even when WAV was requested). Each cue is trimmed or looped in FFmpeg to its segment plus a 1.5s overlap, then the cues are crossfaded. Muted segments become silence and aren't requested. Each cue is saved under its request fingerprint, so a retry never pays twice. Provider names are kept out of UI copy and error messages. Royalty-free import still works.
...
- Paid smoke tests for the third pass: OpenRouter `/images` with `input_references` (reference thumbnail edits and art-style matching) are built from their documented request shapes but haven't run against the live API. Lyria 3 Pro was verified live on 2026-09-23: a 24s three-segment soundtrack (one muted) composed in 45s, measured exactly 24.0s with a silent muted range, and a rerun reused both cues in 0.45s with no new requests.
