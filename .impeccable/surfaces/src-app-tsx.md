...
The Tools index presents five exact 9:16 cards in two columns on mobile and five columns on wide desktop. Cards use quiet illustrated motifs, a direct “Open tool” action, and no tags.
...
- `tools-desktop.png` and `chat-desktop.png` validate the 1440×1000 desktop frame.
- `tools-mobile.png` and `chat-mobile.png` validate the 390×844 responsive card and composer composition. The final Activity integration was validated in code after moving it from the content plane into the mobile workspace header and Agent tools.
